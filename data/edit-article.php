...
<form action="./data/insert-article.php" method="post" class="editorform">
 ...
 ?></div>
	...
	value="<?php  if (isset($row['headimage'])) echo $row['headimage']; ?>" />
	<div class="control-group">
		<label class="control-label" for="inputTitle">タイトル</label>
		<div class="controls">
			<input type="text" name="title" id="inputTitle" style="font-size: 24px; height: 28px; width: 98%;" value="<?php if (isset($row['title'])) echo $row['title']; ?>" />
		</div>
	</div>
	<div class="control-group">
		<label class="control-label" for="editor">本文</label>
		<div class="controls">
			<textarea name="body" id="editor" ><?php if (isset($row['body'])) echo $row['body']; ?></textarea>
		</div>
	</div>
	<div class="control-group">
		<label class="control-label" for="inputTag">タグ</label>
		<div class="controls">
			<input type="text" name="tag" id="inputTag" value="<?php if (isset($row['tag'])) echo $row['tag'] ?>" />
		</div>
	</div>
	<input type="hidden"
	name="rowid" value="<?php if (isset($row['id'])) echo $row['id']; ?>" />
	<input type="hidden"
	name="author" value="<?php if (isset($row['author'])) echo $row['author']; ?>" />
	<hr />
	<div class="control-group">
		<div class="controls">
